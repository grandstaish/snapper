...
internal data class TargetType(
  ...
  companion object {
    ...
    fun get(messager: Messager, elements: Elements, types: Types, element: Element): TargetType? {
      ...
      if (element !is TypeElement || typeMetadata !is KotlinClassMetadata) {
        messager.printMessage(
            ERROR, "@Serializable can't be applied to $element: must be a Kotlin class", element)
        return null
      }
      ...
      when {
        proto.classKind == Class.Kind.ENUM_CLASS -> {
          messager.printMessage(
              ERROR, "@Serializable can't be applied to $element: must not be an enum class",
              element)
          return null
        }
        ...
        proto.isInnerClass -> {
          messager.printMessage(
              ERROR, "@Serializable can't be applied to $element: must not be an inner class",
              element)
          return null
        }
        proto.modality == ABSTRACT -> {
          messager.printMessage(
              ERROR, "@Serializable can't be applied to $element: must not be abstract", element)
          return null
        }
        proto.visibility == LOCAL -> {
          messager.printMessage(
              ERROR, "@Serializable can't be applied to $element: must not be local", element)
          return null
        }
      }
      ...
      if (constructor.proto.visibility != INTERNAL && constructor.proto.visibility != PUBLIC) {
        messager.printMessage(ERROR, "@Serializable can't be applied to $element: " +
      ...
      for (supertype in appliedType.supertypes(types)) {
        ...
        if (supertype.element.kotlinMetadata == null) {
          messager.printMessage(ERROR,
              "@Serializable can't be applied to $element: supertype $supertype is not a Kotlin type",
              element)
          return null
        }
    ...
    private fun genericTypeNames(proto: Class, nameResolver: NameResolver): List<TypeVariableName> {
      return proto.typeParameterList.map { parameter ->
        val possibleBounds = parameter.upperBoundList
        ...
        val typeVar = if (possibleBounds.isEmpty()) {
          TypeVariableName(
              ...
              variance = parameter.varianceModifier)
        } else {
          TypeVariableName(
              name = nameResolver.getString(parameter.name),
        ...
        return@map typeVar.reified(parameter.reified)
      }
    }
